# Disconnected OpenShift Installs

> For Disconnected Mirroring automation see https://github.com/kenmoini/ocp4-disconnected-helper

## JFrog Registry Passthrough Mode (ADR 0020)

For deployments using JFrog Artifactory with passthrough mode, use the automation in `ocp4-disconnected-helper`:

```bash
# Deploy JFrog with passthrough mode
airflow dags trigger ocp_jfrog_agent_deployment \
    --conf '{"cluster_name": "ocp-jfrog", "base_domain": "example.com", "ocp_version": "4.20.4"}'
```

See `examples/jfrog-disconnected/` for cluster configuration examples.

## Post Cluster Deployment Configuration

### 1. Create a ConfigMap in `openshift-config` with the additionalTrustBundle

```yaml=
apiVersion: v1
kind: ConfigMap
metadata:
  name: additional-trust-bundle
  namespace: openshift-config
data:
  ca-bundle.crt: |
    -----BEGIN CERTIFICATE-----
    
    S5kIfhw8FM52x6RHCwRicArO8HSTCf4ueEkFL7yj5xSI
    -----END CERTIFICATE-----
  # JFrog Registry Certificate (replace with your generated cert)
  jfrog-registry: |
    # Certificate will be generated by setup-certificates.yml
    # or step-ca during deployment
    # Path: /etc/pki/disconnected-ca/ca.crt
```

> **Note**: For JFrog passthrough deployments, the certificate is automatically generated by the `ocp_jfrog_agent_deployment` DAG using either step-ca or self-signed certificates (ADR 0016). The CA certificate will be at `/etc/pki/disconnected-ca/ca.crt`.

### 2. Set the Proxy config to the name of that ConfigMap

```yaml=
apiVersion: config.openshift.io/v1
kind: Proxy
metadata:
  name: cluster
spec:
  trustedCA:
    name: 'additional-trust-bundle'
```

### 3. Configure the Image Cluster Config with:

```yaml=
spec:
  additionalTrustedCA:
    name: additional-trust-bundle
```

### 4. Delete the ImageContentSourcePolicies
### 5. Configure ImageTagMirrorSet and ImageDigestMirrorSet - see templated manifests
### 6. Create OpenShift Update Service